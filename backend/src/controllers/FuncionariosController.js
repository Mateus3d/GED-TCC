...
module.exports = {
  ...
  async store(req, res) {
    ...
    let funcionario = await Funcionario.findOne({ username })  //verifica se o funcionario já existe
       .catch(e =>{
         console.error(e)
    ...
    if (!funcionario) { //se ñ encontrar o funcionario entao cria
      ...
      return res.status(400).json({error: 'Username inválido, já existe outro funcionário com esse login'})
    }
